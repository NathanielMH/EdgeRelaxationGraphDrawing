...
class ForceAtlas2:
    ...
    def init(self,
        ...
        if isinstance(G, numpy.ndarray):
            ...
            assert numpy.all(
                G.T == G), "G is not symmetric.  Currently only undirected graphs are supported"
            assert isinstance(pos, numpy.ndarray) or (
                pos is None), "Invalid node positions"
    ...
    def forceatlas2(self,
        ...
        for _i in niters:
            ...
            fa2util.apply_gravity(
                nodes, self.gravity, scalingRatio=self.scalingRatio, useStrongGravity=self.strongGravityMode)
            gravity_timer.stop()
