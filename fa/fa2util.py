    ...
    if edgeWeightInfluence == 0:
        for edge in edges:
            linAttraction(nodes[edge.node1], nodes[edge.node2],
                          1, distributedAttraction, coefficient)
    elif edgeWeightInfluence == 1:
        for edge in edges:
            linAttraction(nodes[edge.node1], nodes[edge.node2],
                          edge.weight, distributedAttraction, coefficient)
    else:
        for edge in edges:
...
class Region:
    ...
    def updateMassAndGeometry(self):
        if len(self.nodes) > 1:
            ...
            self.size = 0.0
            for n in self.nodes:
                distance = sqrt((n.x - self.massCenterX) **
                                2 + (n.y - self.massCenterY) ** 2)
...
def adjustSpeedAndApplyForces(nodes, speed, speedEfficiency, jitterTolerance):
    ...
    for n in nodes:
        swinging = n.mass * \
            sqrt((n.old_dx - n.dx) * (n.old_dx - n.dx) +
                 (n.old_dy - n.dy) * (n.old_dy - n.dy))
