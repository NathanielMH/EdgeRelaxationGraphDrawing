...
def preprocess_data(df: pd.DataFrame, return_labels: bool = True, drop_labels: bool = True):
    ...
    df = df.drop(columns=['edge_id', 'graph_id', 'num_nodes', 'num_edges','benchmark', 'max_deg', 'min_deg', 'is_bridge','grad_diff'],inplace=False)
...
def evaluate_accuracy(yn_test: np.array, yn_res: np.array):
    ...
    """
    y_pred = np.array([1 if yn_res[i] > 0.5 else 0 for i in range(len(yn_res))])
    f1 = f1_score(yn_test, y_pred)
    pr = precision_score(yn_test, y_pred)
    re = recall_score(yn_test, y_pred)
    acc = (sum([1 if yn_test[i] == y_pred[i]
...
def main_xgb(alg: str = 'kk'):
    """Main function."""
    df = pd.read_csv('graph_train_'+alg+'.csv')
    Xn, yn = preprocess_data(df)
    Xn_train, Xn_test, yn_train, yn_test = train_test_split(
        Xn, yn, shuffle=True, random_state=42)
